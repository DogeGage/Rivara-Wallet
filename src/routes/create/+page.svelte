...
<script lang="ts">
  ...
  async function generateWallet() {
    ...
    try {
      ...
      if (!window.cryptoLibs) {
        throw new Error(
          "Crypto libraries not loaded. Please refresh the page.",
        );
      }
  ...
  async function verifyAndCreate() {
    ...
    if (!correct) {
      error =
        "Words do not match. Please check your seed phrase and try again.";
      return;
    }
...
<div
  class="min-h-screen bg-[#070b10] flex items-center justify-center p-4 relative overflow-hidden"
>
  <!-- Animated Background -->
  <div class="absolute inset-0 overflow-hidden">
    <div
      class="absolute w-96 h-96 bg-cyan-500/20 rounded-full blur-3xl -top-48 -left-48 animate-pulse"
    ></div>
    <div
      class="absolute w-96 h-96 bg-cyan-500/20 rounded-full blur-3xl -bottom-48 -right-48 animate-pulse"
  ...
  <div class="relative w-full max-w-2xl">
    <div
      class="bg-stone-900/50 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl"
    >
      {#if step === 1}
        <!-- Step 1: Set Password -->
        ...
        <div
          class="mb-6 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg"
        >
          <div class="flex gap-3">
            <span class="text-2xl">⚠️</span>
            <div class="text-sm text-yellow-200">
              <strong>Important:</strong> Write down your seed phrase and keep it
              safe. You'll need it to recover your wallet. Never share it with anyone!
            </div>
          </div>
        </div>
        ...
        <form on:submit|preventDefault={generateWallet} class="space-y-6">
          <div>
            ...
            <p class="text-xs text-slate-500 mt-1">
              Min 12 chars: uppercase, lowercase, number, and symbol required
            </p>
          </div>
        ...
        <div class="mt-6 text-center text-sm text-slate-400">
          Already have a wallet? <button
            ...
            on:click={() => goto("/import")}>Import Wallet</button
          >
        </div>
      {:else if step === 2}
      ...
      {:else}
        ...
        <div class="text-center mb-8">
          ...
          <p class="text-slate-400">
            Fill in the missing words to verify you wrote it down
          </p>
        </div>

        <div
          class="mb-6 p-4 bg-cyan-500/10 border border-cyan-500/20 rounded-lg text-center"
        >
          <p class="text-sm text-cyan-200">
            Enter words #{verifyIndices.map((i) => i + 1).join(", ")} from your seed
            phrase
          </p>
        </div>
        ...
        {#if error}
          <div
            class="p-4 mb-4 bg-red-500/10 border border-red-500/20 rounded-lg text-red-400 text-sm"
          >
            {error}
          </div>
        {/if}
