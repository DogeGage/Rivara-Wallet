...
<script lang="ts">
  ...
  import { interceptDevModeSend } from "$lib/services/dev-mode-service";
        ...
        const ethereumAssets = [
          {
            ...
            usd: convertCurrency(
              currentWallet.ethereum?.balanceUSD || "0",
              currentCurrency,
              currentRates,
            ),
            icon: "/assets/crypto/SVG/eth.svg",
          ...
          },
          {
            ...
            usd: convertCurrency(
              currentWallet.polygon?.balanceUSD || "0",
              currentCurrency,
              currentRates,
            ),
            icon: "/assets/crypto/SVG/matic.svg",
          ...
          },
          {
            ...
            usd: convertCurrency(
              currentWallet.avalanche?.balanceUSD || "0",
              currentCurrency,
              currentRates,
            ),
            icon: "/assets/crypto/SVG/avax.svg",
          ...
          },
          {
            ...
            usd: convertCurrency(
              currentWallet.bsc?.balanceUSD || "0",
              currentCurrency,
              currentRates,
            ),
            icon: "/assets/crypto/SVG/bnb.svg",
          ...
          },
          {
            ...
            usd: ethUsdc
              ? convertCurrency(
                  ethUsdc.balanceUSD || "0",
                  currentCurrency,
                  currentRates,
                )
              : "0.00",
            icon: "/assets/crypto/SVG/iusdc.svg",
            address: currentWallet.ethereum?.address || "",
          },
          {
            id: "USDC_POL",
            ...
            usd: polyUsdc
              ? convertCurrency(
                  polyUsdc.balanceUSD || "0",
                  currentCurrency,
                  currentRates,
                )
              : "0.00",
            icon: "/assets/crypto/SVG/plUSDC.svg",
            address: currentWallet.polygon?.address || "",
          },
        ];
            ...
            assets: [
              {
                ...
                usd: convertCurrency(
                  currentWallet.bitcoin?.balanceUSD || "0",
                  currentCurrency,
                  currentRates,
                ),
                icon: "/assets/crypto/SVG/btc.svg",
              ...
              },
              {
                ...
                usd: convertCurrency(
                  currentWallet.dogecoin?.balanceUSD || "0",
                  currentCurrency,
                  currentRates,
                ),
                icon: "/assets/crypto/SVG/doge.svg",
              ...
              },
              {
                ...
                usd: convertCurrency(
                  currentWallet.litecoin?.balanceUSD || "0",
                  currentCurrency,
                  currentRates,
                ),
                icon: "/assets/crypto/SVG/ltc.svg",
            ...
            assets: [
              {
                ...
                usd: convertCurrency(
                  currentWallet.tron?.balanceUSD || "0",
                  currentCurrency,
                  currentRates,
                ),
                icon: "/assets/crypto/SVG/trx.svg",
            ...
            assets: [
              {
                ...
                usd: convertCurrency(
                  currentWallet.solana?.balanceUSD || "0",
                  currentCurrency,
                  currentRates,
                ),
                icon: "/assets/crypto/SVG/sol.svg",
  ...
  $: selectedAsset = selectedCrypto
    ? allCryptos.find((c) => c.id === selectedCrypto)
    : null;
  ...
  async function selectAsset(id: string) {
    ...
    await Promise.all([loadTransactions(id), loadChart(id, chartDays)]);
  }
    ...
    ? addressBookService.getByChain(
        selectedAsset.id === "BTC"
          ...
          : selectedAsset.id === "ETH"
            ? "ethereum"
            : selectedAsset.id === "POL"
              ? "polygon"
              : selectedAsset.id === "DOGE"
                ? "dogecoin"
                : selectedAsset.id === "LTC"
                  ? "litecoin"
                  : selectedAsset.id === "SOL"
                    ? "solana"
                    : selectedAsset.id === "AVAX"
                      ? "avalanche"
                      : selectedAsset.id === "BNB"
                        ? "bsc"
                        : selectedAsset.id === "TRX"
                          ? "tron"
                          : "",
      )
    : [];
  ...
  function reviewTransaction() {
    if (!sendAddress.trim()) {
      sendError = "Please enter a recipient address";
      return;
    }
    ...
    // 🐰 Easter Eggs
    if (sendAddress.trim() === "1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa") {
      sendError = "Satoshi doesn't need your sats! HODL them. 👑";
      return;
    }
    if (sendAmount.toString() === "420.69" && selectedAsset.id === "DOGE") {
      sendError = "Nice. 🐕💨";
      setTimeout(() => sendError = "", 2000);
    } else {
      sendError = "";
    }
  ...
  async function confirmSend() {
    ...
    try {
      ...
      if (selectedAsset.id === "USDC_ETH") {
        ...
        // ── Dev mode magic send ──────────────────────────────────────────
        if (interceptDevModeSend(chainKey, sendAddress)) {
          // Not a real transaction — show a special toast instead of success
          sendStage = "form";
          resetSendForm();
          // Brief notification using the existing error slot (re-styled in UI)
          sendError = "__devmode__";
  ...
  async function loadChart(assetId: string, days: number) {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.rivarawallet.xyz/api/coingecko/chart?id=${coinId}&days=${days}`,
      );
  ...
  function getServiceForAsset(assetId: string) {
    if (["BTC", "DOGE", "LTC"].includes(assetId)) {
      ...
      return walletService.getChainService(
        assetId === "POL" || assetId === "USDC_POL" ? "polygon" : "ethereum",
      );
    } else if (assetId === "SOL") {
...
<div class="flex flex-col h-screen bg-white dark:bg-[#070b10]">
  ...
  <div class="flex flex-1 overflow-hidden">
    <!-- Left Sidebar -->
    <aside
      class="w-80 bg-stone-900/30 backdrop-blur-xl border-r border-white/5 flex flex-col max-md:w-full max-md:border-r-0 {selectedAsset
        ? 'max-md:hidden'
        : ''}"
    >
      <!-- Balance Header -->
      <div
        class="p-6 border-b border-white/5 bg-gradient-to-b from-white/[0.02] to-transparent"
      >
        <div class="text-3xl font-extrabold text-white mb-1 tracking-tight">
          {currencySymbols[currentCurrency]}{convertedBalance}
          <span class="text-sm text-slate-500 font-medium"
            >{currentCurrency.toUpperCase()}</span
          >
        </div>
        <div class="text-xs text-slate-500 font-medium">
          8 chains{isLoading ? " • Loading..." : ""}
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto py-2">
        {#each chainGroups as group}
          <div
            class="mx-3 mb-3 rounded-lg bg-white/[0.03] border border-transparent hover:bg-white/[0.06] hover:border-white/10 transition-all overflow-hidden"
          >
            <!-- Chain Header -->
            <button
            ...
            >
              <div class="flex items-center gap-2.5">
                ...
                <div class="text-left">
                  ...
                  <div class="text-xs text-slate-500">
                    {group.assets.length} asset{group.assets.length > 1
                      ? "s"
                      : ""}
                  </div>
                </div>
              ...
              <div class="flex items-center gap-2.5">
                <div class="text-sm font-bold text-white">
                  ${group.assets
                    .reduce((sum, a) => sum + parseFloat(a.usd), 0)
                    .toFixed(2)}
                </div>
                <div
                  class="text-slate-500 text-xs transition-transform duration-300"
                  style="transform: rotate({expandedChains[group.key]
                    ? 90
                    : 0}deg)"
                >
            ...
            {#if expandedChains[group.key]}
              <div class="bg-black/20 border-t border-white/5">
                {#each group.assets as asset}
                  <button
                    class="w-full flex items-center gap-2.5 px-3 py-2.5 pl-11 border-b border-white/[0.02] last:border-b-0 transition-all hover:bg-white/5 {selectedCrypto ===
                    asset.id
                      ? 'bg-cyan-500/15 border-l-3 !border-l-cyan-500'
                      : ''}"
    ...
    {#if selectedAsset}
      <main
        class="flex-1 overflow-y-auto p-6 pb-24 md:pb-6 {selectedAsset
          ? 'max-md:block'
          : 'max-md:hidden'}"
      >
        <div class="w-full">
          ...
          <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex gap-2.5">
              <button
                class="px-5 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-sm font-medium transition-all hover:bg-white/10 {!showingSend
          ...
          {#if !showingSend}
            <!-- Balance Card -->
            <div
              class="mb-6 p-8 rounded-xl bg-gradient-to-br from-slate-900/40 to-slate-800/60 backdrop-blur-xl border border-white/5 border-t-white/10 shadow-2xl"
            >
              <div class="mb-5">
                ...
                {#if balanceVisible}
                  <div
                    class="text-5xl font-extrabold text-white mb-2 tracking-tight"
                  >
              ...
              <div class="pt-5 border-t border-white/5">
                <div class="text-sm text-slate-500 mb-2.5 font-medium">
                  Wallet Address
                </div>
                <div
                  class="flex items-center gap-2.5 p-3.5 bg-black/20 rounded-lg border border-white/5"
                >
                  <span
                    class="flex-1 text-sm text-slate-300 font-mono break-all"
                    >{selectedAsset.address}</span
                  >
                  <button
                    on:click={() => (showingQR = true)}
                    class="p-2 hover:bg-white/5 rounded-lg transition"
                    title="Show QR Code"
                  >
            ...
            <!-- Price Chart -->
            <div
              class="mb-6 p-6 rounded-xl bg-gradient-to-br from-slate-900/40 to-slate-800/60 backdrop-blur-xl border border-white/5 border-t-white/10 shadow-2xl"
            >
              <div class="flex items-center justify-between mb-5">
                <h2 class="text-lg font-bold text-white">Price Chart</h2>
                <div class="flex gap-1.5">
                  <button
                    class="px-2.5 py-1.5 text-xs font-medium rounded-md transition {chartDays ===
                    1
                      ? 'bg-cyan-600 text-white'
                      : 'bg-stone-800/50 text-slate-400 hover:bg-stone-700'}"
                    on:click={() => changeChartTimeframe(1)}>24H</button
                  >
                  <button
                    class="px-2.5 py-1.5 text-xs font-medium rounded-md transition {chartDays ===
                    7
                      ? 'bg-cyan-600 text-white'
                      : 'bg-stone-800/50 text-slate-400 hover:bg-stone-700'}"
                    on:click={() => changeChartTimeframe(7)}>7D</button
                  >
                  <button
                    class="px-2.5 py-1.5 text-xs font-medium rounded-md transition {chartDays ===
                    30
                      ? 'bg-cyan-600 text-white'
                      : 'bg-stone-800/50 text-slate-400 hover:bg-stone-700'}"
                    on:click={() => changeChartTimeframe(30)}>30D</button
                  >
                </div>
              </div>
              {#if loadingChart}
                <div
                  class="h-40 flex items-center justify-center text-slate-500 text-sm"
                >
                  Loading chart...
                </div>
            ...
            <!-- Transactions -->
            <div
              class="p-6 rounded-xl bg-gradient-to-br from-slate-900/40 to-slate-800/60 backdrop-blur-xl border border-white/5 border-t-white/10 shadow-2xl"
            >
              ...
              {:else if transactions.length > 0}
                <div class="space-y-2">
                  {#each transactions as tx}
                    <div
                      class="flex items-center gap-3 p-3 rounded-lg bg-black/20 border border-white/5 hover:bg-white/5 transition"
                    >
                      <div
                        class="w-8 h-8 rounded-full flex items-center justify-center {tx.type ===
                        'sent'
                      ...
                      <div class="text-right">
                        <div
                          class="text-sm font-semibold {tx.type === 'sent'
                            ? 'text-red-400'
                            : 'text-green-400'}"
                        >
          ...
          {:else}
            <!-- Send Form -->
            <div
              class="p-8 rounded-xl bg-gradient-to-br from-slate-900/40 to-slate-800/60 backdrop-blur-xl border border-white/5 border-t-white/10 shadow-2xl"
            >
              <div class="max-w-2xl">
                {#if sendStage === "form"}
                  <div class="mb-6">
                    <div class="flex items-center justify-between mb-2.5">
                      <label class="block text-sm text-slate-400 font-semibold"
                    ...
                    {#if showContactPicker && sendContacts.length > 0}
                      <div
                        class="mb-2 bg-black/30 border border-white/10 rounded-lg overflow-hidden"
                      >
                        {#each sendContacts as c}
                          <button
                            class="w-full px-4 py-2.5 text-left hover:bg-white/5 transition border-b border-white/5 last:border-0"
                            on:click={() => {
                              sendAddress = c.address;
                  ...
                  <div class="mb-6">
                    <label
                      class="block text-sm text-slate-400 mb-2.5 font-semibold"
                      >Amount</label
                    >
                    <div class="relative">
                      <input
                      ...
                      />
                      <span
                        class="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 font-semibold text-sm"
                    ...
                    >
                      ...
                      >
                      <button
                        on:click={() => (sendAmount = selectedAsset.balance)}
                        class="px-2.5 py-1 border border-cyan-500/50 text-cyan-400 rounded-md hover:bg-cyan-500/10 transition font-medium text-xs"
                        >MAX</button
                      >
                    </div>
                  </div>
                  ...
                  <div class="mb-6">
                    ...
                    >
                    <div
                      class="p-3.5 bg-black/20 border border-white/10 rounded-lg"
                    >
                      <span class="text-slate-400 text-sm"
                        >Estimated fee: ~$0.50</span
                      >
                    </div>
                  </div>

                  <div
                    class="mb-6 p-5 bg-black/20 border border-white/10 rounded-lg"
                  >
                    <div
                      class="flex justify-between items-center py-2 border-b border-white/5 mb-2"
                    >
                      <span class="text-slate-400 text-sm">You're sending</span>
                      <span class="text-white font-semibold text-sm"
                        >{sendAmount || "0"} {selectedAsset.symbol}</span
                      >
                    </div>
                  ...
                  {#if sendError === "__devmode__"}
                    <div
                      class="mb-4 p-3 bg-cyan-500/10 border border-cyan-500/30 rounded-lg text-cyan-300 text-sm font-mono flex items-center gap-2"
                    >
                      🛠️ Dev Mode toggled! Check Settings → Dev Mode.
                    </div>
                  {:else if sendError}
                    <div
                  ...
                  <button
                    on:click={reviewTransaction}
                    class="w-full py-3.5 bg-gradient-to-r from-cyan-600 to-cyan-600 text-white font-bold rounded-lg hover:from-cyan-500 hover:to-cyan-500 transition-all shadow-lg shadow-cyan-500/25"
                  >
                    Review Transaction
                  </button>
                {:else if sendStage === "confirm"}
                  ...
                  <div
                    class="mb-6 p-5 bg-black/20 border border-white/10 rounded-lg space-y-3"
                  >
                    <div class="flex justify-between items-start">
                      <span class="text-slate-400 text-sm">To</span>
                      <span
                        class="text-white font-mono text-xs text-right max-w-[60%] break-all"
                        >{sendAddress}</span
                      >
                    </div>
                    <div
                      class="border-t border-white/5 pt-3 flex justify-between items-center"
                    >
                      <span class="text-slate-400 text-sm">Amount</span>
                      <span class="text-white font-semibold"
                        >{sendAmount} {selectedAsset.symbol}</span
                      >
                    </div>
                    <div
                      class="border-t border-white/5 pt-3 flex justify-between items-center"
                  ...
                  <div class="flex gap-3">
                    <button
                      on:click={resetSendForm}
                      class="flex-1 py-3.5 bg-white/5 border border-white/10 text-white font-bold rounded-lg hover:bg-white/10 transition"
                      >Back</button
                    >
                    <button
                      on:click={confirmSend}
                      class="flex-1 py-3.5 bg-gradient-to-r from-cyan-600 to-cyan-600 text-white font-bold rounded-lg hover:from-cyan-500 hover:to-cyan-500 transition-all shadow-lg shadow-cyan-500/25"
                      >Confirm & Send</button
                    >
                  </div>
                {:else if sendStage === "sending"}
                  <div class="py-12 text-center">
                    <div
                      class="w-12 h-12 border-2 border-cyan-500 border-t-transparent rounded-full animate-spin mx-auto mb-4"
                    ></div>
                ...
                {:else if sendStage === "error"}
                  <div class="py-8 text-center">
                    <div
                      class="w-14 h-14 rounded-full bg-red-500/10 border border-red-500/20 flex items-center justify-center mx-auto mb-4 text-2xl"
                    >
                      ⚠️
                    </div>
                    ...
                    <button
                      on:click={resetSendForm}
                      class="px-6 py-3 bg-white/5 border border-white/10 text-white font-semibold rounded-lg hover:bg-white/10 transition"
                      >Try Again</button
                    >
                  </div>
                {:else if sendStage === "success"}
                  <div class="py-8 text-center">
                    <div
                      class="w-14 h-14 rounded-full bg-green-500/10 border border-green-500/20 flex items-center justify-center mx-auto mb-4 text-3xl"
                    >
                      ✓
                    </div>
                    <p class="text-white font-semibold mb-2 text-xl">
                      Transaction Sent!
                    </p>
                    ...
                    {#if sendTxHash}
                      <div
                        class="mb-6 p-4 bg-black/20 border border-white/10 rounded-lg"
                      >
    ...
    {:else}
      ...
      <main
        class="flex-1 flex items-center justify-center text-slate-500 max-md:hidden"
      >
